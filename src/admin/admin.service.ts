  ...
  async getModerators(
    ...
    return {
      data: moderators.map((moderator) =>
        this.moderatorMapper.toModeratorDto(moderator),
      ),
      error: false,
    };
  }
  ...
  private async getStatistics(
    ...
    const data: StatisticsItemDto[] = periods.map((periodStart) => {
      const totalUsers = usersRaw
        .filter(
          ({ period: registrationPeriod }) => registrationPeriod <= periodStart,
        )
      ...
      return {
        ...
        total_presentation_duration_seconds: Math.round(
          presMap.get(periodStart.toISOString()) ?? 0,
        ),
